...
describe("CaseForm Component Davranış Testleri (FINAL-CASE-03)", () => {
  ...
  it("AC-01: ilk render edildiğinde 10 form kontrolü görünür ve hata özeti bulunmaz", () => {
    ...
    expect(screen.getByLabelText(/içerik dili|dil|language/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/hatırlanan ayrıntı|details/i)).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/önceki aramalar|searches/i)).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("button", { name: /vaka oluştur|gönder|kaydet|submit/i }),
    ).toBeInTheDocument();
